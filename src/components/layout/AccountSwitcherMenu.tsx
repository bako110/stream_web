...
import { PlusCircle, Check, User as UserIcon } from 'lucide-react';
...
/** Contenu partagé du menu de bascule de compte (liste des comptes stockés +
 * "Voir mon profil" + "Ajouter un compte") — extrait de AccountSwitcherDropdown
 * pour être réutilisé tel quel dans un contexte différent (dropdown flottant
 * desktop via createPortal, ou bottom-sheet/liste inline mobile dans
 * MobileDrawer.tsx). Le composant appelant gère uniquement le déclencheur et
 * le conteneur (positionnement, fermeture au clic extérieur, etc.). */
export function AccountSwitcherMenu({ accounts, onClose }: { accounts: StoredAccount[]; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <button
        onClick={() => { onClose(); navigate('/auth/login?mode=add'); }}
        disabled={!canAdd}
