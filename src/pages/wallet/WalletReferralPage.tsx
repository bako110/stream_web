...
export default function WalletReferralPage() {
  ...
  const handleShare = useCallback(async () => {
    if (!stats?.referral_code) return;
    const text = `Rejoins-moi sur FoliX ! Utilise mon code de parrainage ${stats.referral_code} lors de ton inscription et gagne 20 coins bonus.`;
    if (navigator.share) {
      await navigator.share({ title: 'Invite un ami sur FoliX', text }).catch(() => {});
    } else {
